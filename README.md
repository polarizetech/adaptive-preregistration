...
A project doesn't link to this repo at runtime. Its CLI, `kit`, **copies** the protocols into the project (`.agents/`) and records the exact kit commit in a lockfile. One command pulls the latest version. Agents always read files that are really in the repo, so the protocols also work in cloud sandboxes, offline, and for collaborators.

```
this repo (the kit)                         a project that uses it
─────────────────────                       ───────────────────────────────────────────
modules/core/          ────kit init────▶    AGENTS.md        managed block: always-on rules
modules/convo-log/                          CLAUDE.md        "@AGENTS.md" (Claude Code import)
modules/prereg/        ◀───kit update───    .agents/         protocols/, tools/, bin/kit, kit.lock
...
~/Sites/kit-adaptive-preregistration/bin/kit init /path/to/project          # core + default modules
...
```bash
.agents/bin/kit status               # installed version, modules, up to date?
.agents/bin/kit update               # pull the latest kit and re-apply
.agents/bin/kit add prereg           # add a module (dependencies come with it)
.agents/bin/kit remove prereg        # remove one: its files, hooks and instructions
kit list                             # what modules exist (run from the kit)
```
...
| GitHub Copilot (VS Code, coding agent) | `AGENTS.md` | `.github/hooks/kit-*.json` (VS Code agent hooks, preview) |
...
`AGENTS.md` is the single source every tool reads. The managed part sits between `<!-- kit:start -->` and `<!-- kit:end -->`. Anything you write outside the markers is project-specific and is never touched.

## Staying up to date

- Each install records `source`, `ref`, `commit` and a sha256 of every managed file in `.agents/kit.lock`.
- The `core` module adds a **session-start hook** that runs `kit check --hook`. If the kit has moved on, or someone edited a managed file, the agent is told to mention it. It never updates on its own. The network check is cached for 6 hours and fails silently when offline.
...
modules/<name>/
  ...
  hooks/copilot.json copied to .github/hooks/kit-<name>.json
...
- Hook commands must reference `.agents/bin/…` or `.agents/tools/…`. That's how the `kit` CLI recognises its own hooks when it updates or removes them.
...
Then add a test in `tests/test_kit.py` and run:

```bash
python3 -m unittest discover tests
```

Try it for real against a scratch repo with `bin/kit init --force /tmp/scratch`. From a kit checkout it installs your working tree, uncommitted changes included, and the lock marks it `dirty`.
