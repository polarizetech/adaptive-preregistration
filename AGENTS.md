...
- `bin/kit_ap` and the module tools use stdlib Python 3.9+ or bash only, and must work on macOS and Linux. Hooks and tools must never block an agent: on failure they warn and exit 0.
...
- Run `python3 -m unittest discover tests` (and `uvx ruff check .`) before committing. Add a test for any new install behaviour.
- Record user-visible changes in `CHANGELOG.md`, saying whether they change behaviour in installed projects. `CONTRIBUTING.md` has the rest of the rules for outside contributors.
