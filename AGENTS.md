...
- `bin/agentkit` and the module tools use stdlib Python 3.9+ or POSIX shell only, and must work on macOS and Linux. Hooks and tools must never block an agent: on failure they warn and exit 0.
- Keep installs backward compatible. An old vendored `.agents/bin/agentkit` runs `update`, then hands off to the new kit's CLI through `_apply`. Don't change the `update` → `_apply` interface or the `kit.lock` fields without handling older locks.
- Run `python3 -m unittest discover tests` before committing. Add a test for any new install behaviour.
- Don't install agentkit into this repo itself. The kit is the source, not a consumer.
