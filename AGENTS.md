...
- `bin/kit` and the module tools use stdlib Python 3.9+ or POSIX shell only, and must work on macOS and Linux. Hooks and tools must never block an agent: on failure they warn and exit 0.
- Keep installs backward compatible. An old vendored `.agents/bin/kit` runs `update`, then hands off to the new kit's CLI through `_apply`. Don't change the `update` → `_apply` interface or the `kit.lock` fields without handling older locks.
